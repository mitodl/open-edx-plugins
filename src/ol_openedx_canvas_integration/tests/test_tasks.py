...
from .helpers import HashableUser, stub_canvas_client_factory
...
def test_sync_user_grade_with_canvas_skips_when_assignment_not_synced(monkeypatch):
    ...
    monkeypatch.setattr(tasks, "CanvasClient", stub_canvas_client_factory(stub_client))
...
def test_sync_user_grade_with_canvas_skips_when_user_not_in_canvas(monkeypatch):
    ...
    stub_client = SimpleNamespace(
        get_canvas_assignments=lambda: {
            "block-v1:MITx+Demo+type@sequential+block@hw1": {
                "id": 201,
                "due_at": None,
            }
        },
    ...
    openedx_user = HashableUser("learner@example.com", user_id=100)
