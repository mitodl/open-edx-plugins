...
@pytest.mark.parametrize(
    ("view_func", "error_message_template", "request_get"),
    [
        pytest.param(
            views.list_canvas_enrollments,
            "No canvas_course_id set for course: {course_key}",
            None,
            id="list_canvas_enrollments",
        ),
        pytest.param(
            views.list_canvas_assignments,
            "No canvas_course_id set for course: {course_key}",
            None,
            id="list_canvas_assignments",
        ),
        pytest.param(
            views.list_canvas_grades,
            "No canvas_course_id set for course {course_key}",
            {"assignment_id": "201"},
            id="list_canvas_grades",
        ),
    ],
)
def test_list_view_raises_when_no_canvas_course_id(
    monkeypatch, view_func, error_message_template, request_get
):
    """Test that list views raise when no canvas_course_id is set for the course."""
    request = MagicMock()
    if request_get is not None:
        request.GET = request_get
    course_key = "course-v1:MITx+Demo+2026"
    course = SimpleNamespace(id=course_key)
    ...
    error_message = error_message_template.format(course_key=course_key)
    with pytest.raises(Exception, match=re.escape(error_message)):
        _call_view(view_func, request, course_id=course_key)


def _configure_enrollments_stub(monkeypatch):
    """Stub CanvasClient plus the User/enrollment lookups the enrollments view needs."""
    enrollment_dict = {
        "user1@example.com": {"id": 101},
        "user2@example.com": {"id": 102},
    }
    stub_client = SimpleNamespace(list_canvas_enrollments=lambda: enrollment_dict)
    monkeypatch.setattr(views, "CanvasClient", _stub_canvas_client_factory(stub_client))
    monkeypatch.setattr(
        views.User,
        "objects",
        SimpleNamespace(filter=lambda **_kwargs: SimpleNamespace(first=lambda: None)),
    )
    monkeypatch.setattr(
        views.CourseEnrollmentAllowed,
        "objects",
        SimpleNamespace(filter=lambda **_kwargs: SimpleNamespace(first=lambda: None)),
    )
    monkeypatch.setattr(
...
def _configure_assignments_stub(monkeypatch):
    """Stub CanvasClient.list_canvas_assignments for list_canvas_assignments."""
    assignments = [
        {"id": 201, "name": "Assignment 1"},
        {"id": 202, "name": "Assignment 2"},
    ]
    stub_client = SimpleNamespace(list_canvas_assignments=lambda: assignments)
    monkeypatch.setattr(views, "CanvasClient", _stub_canvas_client_factory(stub_client))


def _configure_grades_stub(monkeypatch):
    """Stub CanvasClient.list_canvas_grades for list_canvas_grades."""
    grades = [
        {"user_id": 101, "score": 95},
        {"user_id": 102, "score": 87},
    ]
    stub_client = SimpleNamespace(list_canvas_grades=lambda **_kwargs: grades)
    monkeypatch.setattr(views, "CanvasClient", _stub_canvas_client_factory(stub_client))


@pytest.mark.parametrize(
    ("view_func", "configure_stub", "request_get"),
    [
        pytest.param(
            views.list_canvas_enrollments,
            _configure_enrollments_stub,
            None,
            id="list_canvas_enrollments",
        ),
        pytest.param(
            views.list_canvas_assignments,
            _configure_assignments_stub,
            None,
            id="list_canvas_assignments",
        ),
        pytest.param(
            views.list_canvas_grades,
            _configure_grades_stub,
            {"assignment_id": "201"},
            id="list_canvas_grades",
        ),
    ],
)
def test_list_view_success(monkeypatch, view_func, configure_stub, request_get):
    """Test that list views return a successful JSON response."""
    request = MagicMock()
    if request_get is not None:
        request.GET = request_get
    ...
    monkeypatch.setattr(views, "get_course_by_id", mock_get_course)
    monkeypatch.setattr(views, "get_canvas_course_id", mock_get_canvas_id)
    configure_stub(monkeypatch)

    response = _call_view(view_func, request, course_id)
...
def test_view_raises_when_no_course_id(monkeypatch, view_func):
    """Test that the view raises when course_id is empty."""
    request = MagicMock()
