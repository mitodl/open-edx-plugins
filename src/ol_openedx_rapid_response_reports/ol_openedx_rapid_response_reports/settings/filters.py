"""Helper to register the Rapid Responses instructor-dashboard tab filter.

Kept separate from ``pipeline`` so it is safe to import at settings-load time,
and shared by both the common and production settings.
"""

INSTRUCTOR_DASHBOARD_TABS_FILTER = (
    "org.openedx.learning.instructor.dashboard.tabs.requested.v1"
)
RAPID_RESPONSE_TAB_PIPELINE_STEP = (
    "ol_openedx_rapid_response_reports.pipeline.AddRapidResponseInstructorTab"
)


def register_instructor_tab_filter(settings):
    """Merge the Rapid Responses tab pipeline step into ``OPEN_EDX_FILTERS_CONFIG``.
    ...
    if RAPID_RESPONSE_TAB_PIPELINE_STEP not in filter_entry["pipeline"]:
        filter_entry["pipeline"].append(RAPID_RESPONSE_TAB_PIPELINE_STEP)
